...
func TestNewAdapter(t *testing.T) {
	...
	cfg := testutil.NewTestEquipmentConfig(types.VendorFiberHome, "10.0.0.1")
...
func TestConnect_Delegates(t *testing.T) {
	...
	adapter := NewAdapter(mock, testutil.NewTestEquipmentConfig(types.VendorFiberHome, "10.0.0.1"))
	cfg := testutil.NewTestEquipmentConfig(types.VendorFiberHome, "10.0.0.1")
...
func TestCreateSubscriber_Success(t *testing.T) {
	mock := &testutil.MockDriver{Connected: true}
	adapter := NewAdapter(mock, testutil.NewTestEquipmentConfig(types.VendorFiberHome, "10.0.0.1"))
	sub := testutil.NewTestSubscriber("FIBR12345678", "0/1", 100)
	...
	if result.Metadata["vendor"] != "fiberhome" {
		t.Fatalf("expected vendor=fiberhome, got %v", result.Metadata["vendor"])
	}
}
